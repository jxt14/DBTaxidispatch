...
const bool Optimization_KNN_Cut=false;//是否开启KNN剪枝查询算法
...
struct Graph//无向图结构 
{
	...
	vector<int>con;//连通性
	vector<int> Split(Graph *G[],idx_t nparts)//将子图一分为二返回color数组，并将两部分分别存至G1，G2 METIS algorithm,npart表示划分块数
	{
	...
	int Split_Borders(idx_t nparts)//将该图划分为nparts块后会产生的border数
	{
		if(n<Naive_Split_Limit)
		{
...
int main()
{
	...
	TaxiDispatch ths;
	ths.Init();
	for (int i = 0 ; i < ths.carnum; i++){
		Car nc = ths.dcar[i];
		for (int j = 1; j <= nc.cusnum; j++)nc.dsef[j] = tree.search(nc.sef.num, nc.customer[j].num);
		for (int j = 1; j <= nc.cusnum; j++){
			nc.dc[j][j] = 0;
			for (int k = j + 1; k <= nc.cusnum; k++){
				nc.dc[j][k] = tree.search(nc.customer[j].num, nc.customer[k].num);
				nc.dc[k][j] = nc.dc[j][k];
			}
		}
		ths.dcar[i] = nc;
	...
	ofstream fout;
	fout.open("cus.txt");
	fout << ths.carnum << endl;
	for (int i = 0; i < ths.carnum; i++){
		Car nc = ths.dcar[i];
		fout << nc.cusnum << endl;
		for (int j = 1; j <= nc.cusnum; j++)fout << nc.dsef[j] << " ";
		fout << endl;
		for (int j = 1; j <= nc.cusnum; j++){
			for (int k = 1; k <= nc.cusnum; k++)fout << nc.dc[j][k] << " ";
			fout << endl;
		}
	}
	fout.close();
	printf("car customer pre finish\n");
	{
		printf("start query\n");
		TIME_TICK_START
